...
void Circuit::AddComponent(Component& component) {
    ...
    adjacencyList[component.fromNode].emplace_back(component.toNode, index);
    adjacencyList[component.toNode].emplace_back(component.fromNode, index);
}


int Circuit::ConfigureCircuit() {
    ...
    for (const auto& [node, adjacentNodes] : adjacencyList) {
        ...
        for (const auto& [neighboringNode, componentID] : adjacentNodes) {
            ...
            if (comp->type == ComponentType::VoltageSource && comp->fromNode == node) {
                negativeVoltages++;
            }
        }
    ...
    for (const Component* c : components) {
        if (c->type == ComponentType::VoltageSource)
            variables.push_back("I_" + c->label);
...
void Circuit::SolveCircuit(float deltaTime) {
    ...
    for (Component* c : components) {
        int i = (c->fromNode == referenceNode) ? -1 : nodeToIndex[c->fromNode];
        int j = (c->toNode == referenceNode) ? -1 : nodeToIndex[c->toNode];
    ...
    for (Component* c : components) {
        switch (c->type) {
            ...
            case ComponentType::Switch: {
                c->voltage = 0;
                c->current = 0;
                c->isClosed = false; // optional: reset switch to default state
                break;
            }
        }
    }
